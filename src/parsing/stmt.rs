...
use crate::parsing::stmt::Stmt::{ExprStmt, LStmt, VarStmt, FnStmt, LetStmt, FnCurried, ReturnStmt, PrintStmt, TypeAlias, WhileStmt};
...
pub enum Stmt {
    ...
    FnCurried { name: Option<String>, token: Token, param: Pair<LType>, ret: Box<Stmt> } // Name, Some relevant token, param:type pair, return
}
...
impl Display for Stmt {
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        match self {
            ...
            WhileStmt { token: Token, condition, body } => write!(f, "while {} {}", condition, format_block(body)),
